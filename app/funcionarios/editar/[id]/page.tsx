...
import { getFuncionarios, atualizarFuncionario } from "@/lib/mock-api";
...
export default function EditarFuncionarioPage() {
  ...
  const [form, setForm] = useState({
    id: "", nome: "", telefone: "", usuario: "", senha: "", nivelPermissao: "OPERADOR" as NivelPermissao
  });
  ...
  useEffect(() => {
    getFuncionarios().then((data) => {
      ...
      if (f) {
        setForm({
          id: f.id, nome: f.nome, telefone: f.telefone, usuario: f.usuario, senha: f.senha, nivelPermissao: f.nivelPermissao
        });
        if (f.endereco) {
          try {
            const parsed = JSON.parse(f.endereco);
            setEndereco(parsed);
          } catch {
            // Se não for JSON, joga no logradouro pra não perder
            setEndereco(prev => ({ ...prev, logradouro: f.endereco }));
          }
        }
      } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const res = await atualizarFuncionario(id, {
      ...form,
    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 lg:p-8">
          <h1 className="text-xl font-bold text-slate-100 mb-6 font-jockey uppercase tracking-wider">Editar Funcionário</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="ID (não editável)">
                  <input disabled value={form.id} className="input-field opacity-50 cursor-not-allowed" />
                </Field>
                <Field label="Nome">
            ...
            <div className="flex items-center justify-end gap-3 pt-6 border-t border-slate-800">
              ...
              <button type="submit" disabled={submitting} className="px-5 py-2.5 bg-indigo-500 hover:bg-indigo-400 text-white text-sm font-semibold rounded-xl flex items-center gap-2 shadow-lg shadow-indigo-500/20 transition-all">
